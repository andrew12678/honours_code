...
generateDE <- function(cell_type){
  ...
  save_directory = "/albona/nobackup/andrewl/honours_code/combination_matrix_gamma/"
    ...
    for(cells in N_cells){
      for(N_de in N_de_cand){
        for(j in 1:length(de_params)){
          ...
          write.csv(data.frame(gene_index = fc_index), paste0(save_path, id, '_de_genes.csv'))
