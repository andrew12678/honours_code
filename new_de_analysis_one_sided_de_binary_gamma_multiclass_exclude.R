...
performDE = function(cell_type){
  ...
  simulated_path = paste0('combination_matrix_gamma_multiclass_exclude/', cell_type)
  nn_path = paste0('combination_binary_gamma_multiclass_exclude/', cell_type) # change
  save_path = paste0('de_analysis_combination_binary_gamma_multiclass_exclude/', cell_type) # change
  ...
  for(i in 1:nrow(experiments)){
    predictions = as.factor(as.character(read.csv(paste0(nn_path, '/', experiments[i,]$X1))$predictions))
    suffix = experiments[i,]$X2
    matrix_orig = as.matrix(Matrix::readMM(paste0(simulated_path, '/matrix_base_', suffix)))
    matrix_group1 = as.matrix(Matrix::readMM(paste0(simulated_path, '/matrix_sim1_', suffix)))
    matrix_group2 = as.matrix(Matrix::readMM(paste0(simulated_path, '/matrix_sim2_', suffix)))  
    combined_matrix <- as(cbind(matrix_orig, matrix_group1, matrix_group2), "sparseMatrix")
