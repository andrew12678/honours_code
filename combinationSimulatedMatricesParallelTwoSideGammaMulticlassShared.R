...
generateDE <- function(cell_type){
  ...
  save_directory = "/albona/nobackup/andrewl/honours_code/combination_matrix_gamma_multiclass_shared/"
    ...
    for(cells in N_cells){
      for(N_de in N_de_cand){
        for(j in 1:length(de_params)){
          ...
          fc_index2 = sample(setdiff(range, fc_index), N_de/2)
          fc_index2 = c(fc_index2, sample(fc_index, N_de/2))
