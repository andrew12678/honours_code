...
def run_saliency(cell_type):
    ...
    path = f'combination_matrix_gamma_multiclass_shared/{cell_type}'
    data_files = sorted(glob(f'{path}/matrix_base*'))
    gene_names = pd.read_csv(f'{path}/geneNames.csv')
    save_dir = f'combination_binary_gamma_multiclass_shared'
    for header in tqdm(data_files):
        ...
        prediction = []
#         methods = ['Saliency', 'IntegratedGradients',  'InputXGradient', 'GuidedBackprop', 'Deconvolution', 'DeepLift', 'DeepLiftShap', 'GradientShap', 'FeatureAblation', 'Occlusion', 'FeaturePermutation']
...
#                 import ipdb; ipdb.set_trace()
                if method == 'Vanilla':
                    ...
                    if method in {'DeepLiftShap', 'GradientShap'}:
                        base_lines = torch.randn(50, data.shape[1])
        ...
        for i in range(1): # needs to change
            bi = prediction_np == i
            ...
            gene_names_copy.to_csv(f'{save_path}/new_saliency_group_{i}_all_methods.csv')
